...
import { Bell, User, LogOut, Save, X, Users } from "lucide-react";
...
const Cliente = () => {
  ...
  // Estado del formulario para CLIENTE según el diagrama
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      if (parsedUser.rol !== "Administrador") {
        alert("Acceso denegado. Solo los administradores pueden gestionar clientes.");
        navigate("/home");
      }
    } else {
      navigate("/login");
  ...
  const validateForm = () => {
    ...
    if (!emailRegex.test(form.email)) { setError("Ingrese un email válido"); return false; }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = isEditing
        ? `${API_BASE}/api/clientes/${cliente.id_cliente}`
      ...
      <nav className="fixed top-0 w-full bg-white/80 backdrop-blur border-b px-6 py-4 flex justify-between items-center z-50">
        ...
        <div className="hidden md:flex gap-6">
          ...
          <span className="text-[#bc004f] font-bold border-b-2 border-[#bc004f]">
            {isEditing ? "Editar Cliente" : "Agregar Clientes"}
          </span>
        </div>
      ...
      <main className="flex-grow pt-28 px-6 max-w-7xl mx-auto w-full">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-500 mt-2">
            {isEditing ? "Actualiza la información del cliente." : "Registra nuevos clientes en el sistema."}
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          ...
          <section className="lg:col-span-12 bg-white rounded-2xl p-8 shadow-sm border border-gray-200">
            <h2 className="text-lg font-bold mb-6 flex items-center gap-2">
              <span className="text-[#bc004f]">📋</span> Información del Cliente
            </h2>
